    ...
    var barChartData = {
      ...
      datasets: [{
          ...
          data: [
            ...
            while ($t = mysqli_fetch_array($tanggal)) {
              $total = mysqli_fetch_array(mysqli_query($koneksi, "SELECT sum(nanas_a2) as nanas_a2 FROM resume_cannery_a where tanggal='" . $t['tanggal'] . "' group by tanggal"));
            ?> "<?php echo $total['nanas_a2'] ?? 0; ?>",
            <?php
            }
            ?>
        ...
        },
        {
          ...
          data: [
            ...
            while ($b = mysqli_fetch_array($tanggal)) {
              $total = mysqli_fetch_array(mysqli_query($koneksi, "SELECT sum(nanas_a2) as nanas_a2 FROM resume_cannery_b where tanggal='" . $b['tanggal'] . "' group by tanggal"));
            ?> "<?php echo $total['nanas_a2'] ?? 0; ?>",
            <?php
            }
            ?>
        ...
        },
        {
          ...
          data: [<?php
                  ...
                  while ($c = mysqli_fetch_array($tanggal)) {
                    $total = mysqli_fetch_array(mysqli_query($koneksi, "SELECT sum(nanas_a2) as nanas_a2 FROM resume_cannery_all where tanggal='" . $c['tanggal'] . "' group by tanggal"));
                  ?> "<?php echo $total['nanas_a2'] ?? 0; ?>",
            <?php
                  }
            ?>
        ...
        datasets: [{
          ...
          data: [
            ...
            while ($b = mysqli_fetch_array($bulan)) {
              ...
              $total = mysqli_fetch_array(mysqli_query($koneksi, "SELECT (nanas_a2) as nanas_a2 FROM resume_cannery_bulanan where bulan='" . $b['bulan'] . "'"));
            ?> "<?php if (isset($_POST['filter1'])) {
                  echo $b['nanas_a2'] ?? 0;
                } else {
                  echo $total['nanas_a2'] ?? 0;
                } ?> ",
            <?php
