  ...
  datasets: [
    {
      ...
      data: [
                ...
                while($t = mysqli_fetch_array($tanggal)){
                    $total = mysqli_fetch_array(mysqli_query($koneksi,"SELECT sum(choice) as choice FROM resume_cannery_a where tanggal='".$t['tanggal']."' group by tanggal"));
                    ?>
                    "<?php echo $total['choice']??0; ?>",
                    <?php 
                }
                ?>
      ...
      data: [
       <?php 
                ...
                while($b = mysqli_fetch_array($tanggal)){
                    $total = mysqli_fetch_array(mysqli_query($koneksi,"SELECT sum(choice) as choice FROM resume_cannery_b where tanggal='".$b['tanggal']."' group by tanggal"));
                    ?>
                    "<?php echo $total['choice']??0; ?>",
                    <?php 
                }
    ...
    },
    {
      ...
      $tanggal = mysqli_query($koneksi,"SELECT * FROM resume_cannery_all order by tanggal asc");
                ...
                while($c = mysqli_fetch_array($tanggal)){
                    $total = mysqli_fetch_array(mysqli_query($koneksi,"SELECT sum(choice) as choice FROM resume_cannery_all where tanggal='".$c['tanggal']."' group by tanggal"));
                    ?>
                    "<?php echo $total['choice']??0; ?>",
                    <?php 
                }
  ...
  ?>
        ...
        $bulan = mysqli_query($koneksi,"SELECT * FROM resume_cannery_bulanan");
                ...
                while($b = mysqli_fetch_array($bulan)){
                   ...
                   $total = mysqli_fetch_array(mysqli_query($koneksi,"SELECT (choice) as choice FROM resume_cannery_bulanan where bulan='".$b['bulan']."'"));
                    ?>
                    "<?php if(isset($_POST['filter1'])){ 
                      echo $b['choice']??0;
                   }else{echo $total['choice']??0;}?> ",
                    <?php 
