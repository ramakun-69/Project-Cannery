  ...
  <footer class="main-footer">
    ...
    &copy; <?= date('Y') ?> Copyright <strong><span><a href="wa.me/6282244812291">Erasites Citra Digital Indonesia</a> Hosted By <a href="https://idwebhost.com/aff/22028">ID Webhost</a></span></strong>. All Rights Reserved
  </footer>
    ...
    var barChartData = {
      ...
      datasets: [{
          ...
          data: [
            ...
            while ($t = mysqli_fetch_array($tanggal)) {
              $total = mysqli_fetch_array(mysqli_query($koneksi, "SELECT sum(recovery) as recovery FROM resume_cannery_a where tanggal='" . $t['tanggal'] . "' group by tanggal"));
            ?> "<?php echo $total['recovery'] ?? 0; ?>",
            <?php
            }
            ?>
        ...
        },
        {
          ...
          data: [
            ...
            while ($b = mysqli_fetch_array($tanggal)) {
              $total = mysqli_fetch_array(mysqli_query($koneksi, "SELECT sum(recovery) as recovery FROM resume_cannery_b where tanggal='" . $b['tanggal'] . "' group by tanggal"));
            ?> "<?php echo $total['recovery'] ?? 0; ?>",
            <?php
            }
            ?>
        ...
        },
        {
          ...
          data: [<?php
                  ...
                  while ($c = mysqli_fetch_array($tanggal)) {
                    $total = mysqli_fetch_array(mysqli_query($koneksi, "SELECT sum(recovery) as recovery FROM resume_cannery_all where tanggal='" . $c['tanggal'] . "' group by tanggal"));
                  ?> "<?php echo $total['recovery'] ?? 0; ?>",
            <?php
                  }
            ?>
        ...
        datasets: [{
          ...
          data: [
            ...
            while ($b = mysqli_fetch_array($bulan)) {
              ...
              $total = mysqli_fetch_array(mysqli_query($koneksi, "SELECT (recovery) as recovery FROM resume_cannery_bulanan where bulan='" . $b['bulan'] . "'"));
            ?> "<?php if (isset($_POST['filter1'])) {
                  echo $b['recovery'] ?? 0;
                } else {
                  echo $total['recovery'] ?? 0;
                } ?> ",
            <?php
